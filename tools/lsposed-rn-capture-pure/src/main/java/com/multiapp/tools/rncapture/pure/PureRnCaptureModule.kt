package com.multiapp.tools.rncapture.pure
...
/**
 * 纯净 RegisterNatives 捕获模块。
 *
 * 只 hook RegisterNatives 并记录壳的真实注册，不注册任何 stub 方法。
 * 用于从原版 com.qq.reader 进程捕获壳的 YWLoginManager 真实 native 函数指针。
 */
class PureRnCaptureModule : IXposedHookLoadPackage {

    override fun handleLoadPackage(lpparam: XC_LoadPackage.LoadPackageParam) {
        val packageName = lpparam.packageName ?: return
        if (packageName != TARGET_PACKAGE) return

        log("handleLoadPackage package=$packageName process=${lpparam.processName} first=${lpparam.isFirstApplication}")
        installOnce("early")
        thread(name = "pure-rn-capture-installer", isDaemon = true) {
    ...
    private fun installOnce(stage: String) {
        ...
        try {
            ...
            log("install stage=$stage loadedByPath=$loadedByPath nativeOk=$nativeOk rnOk=$rnOk jiaguDiagOk=$jiaguDiagOk installed=${isInstalled.get()}")
        } catch (t: Throwable) {
    ...
    private fun findNativeLibraryDirFromClassLoader(): File? {
        ...
        val apkPath = Regex("/data/app/[^\\s\\]]*com\\.multiapp\\.tools\\.rncapture\\.pure[^\\s\\]]*/base\\.apk")
            .find(text)
            ?.value
            ?: return null
    ...
    companion object {
        private const val TAG = "PURE_RN_CAPTURE"
        private const val TARGET_PACKAGE = "com.qq.reader"
        private const val MODULE_PACKAGE = "com.multiapp.tools.rncapture.pure"
